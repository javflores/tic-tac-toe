defmodule GameEngine.SimpleStrategyTest do
	...
	test "provides available positions to random generator" do
		with_mock GameEngine.PlayStrategies.RandomPositions, [get: fn(_free_positions) -> %{row: 0, column: 1} end] do
			...
			assert called GameEngine.PlayStrategies.RandomPositions.get([%{row: 0, column: 0}, %{row: 2, column: 2}])
		end
	end
end