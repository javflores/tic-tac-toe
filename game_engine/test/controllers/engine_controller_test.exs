defmodule GameEngine.EngineControllerTest do
	...
	test "get empty when game is initialized" do
		...
		with_mock GameEngine.Engine, [:passthrough], [initialize: fn(_engine, _type, _o, _x) -> {:ok, %{game_id: 123, board: %{}, x: "C-3PO", o: "R2-D2"}} end] do
	...
	test "returns new game with id when initializing" do
		...
		with_mock GameEngine.Engine, [:passthrough], [initialize: fn(_engine, _type, _o, _x) -> {:ok, %{game_id: game_id, board: %{}, x: "C-3PO", o: "R2-D2"}} end] do
			response = GameEngine.EngineController.initialize(conn, %{"type" => GameEngine.GameType.computer_computer, "o" => "C-3PO", "x" => "R2-D2"})
	...
	test "get started game based on engine" do
		with_mock GameEngine.Engine, [:passthrough], [start: fn(_engine, _game_id, _first_player) -> {:ok, %{board: %GameEngine.Board{}, next_player: "C-3PO"}} end] do
	...
	test "get error upon game start when engines sends an error" do
		...
		with_mock GameEngine.Engine, [:passthrough], [start: fn(_engine, _game_id, _first_player) -> {:error, expected_error} end] do
	...
	test "get move played by computer" do
		...
		expected_position = 5
		with_mock GameEngine.Engine, [:passthrough], [move: fn(_engine, _game_id) -> 
	...
	test "get status return by engine" do
