defmodule GameEngine.PlayStrategies.KickAssForceDefendingMoves do
	def force_defending(board, player) do
		board 
	...
	def find_corners_in_rows({[nil, :o, nil], row}) when row in [0, 2], do: [%{row: row, column: 0}]
	...
	def find_corners_in_columns({[nil, :o, nil], column}) when column in [0, 2], do: [%{row: 0, column: column}]
	...
	def find_corners_in_diagonals({[nil, :o, nil], 0}), do: [%{row: 0, column: 0}]
	def find_corners_in_diagonals({[:o, nil, nil], 0}), do: [%{row: 2, column: 2}]

	def find_corners_in_diagonals({[nil, nil, :o], 1}), do: [%{row: 0, column: 2}]
	def find_corners_in_diagonals({[nil, :o, nil], 1}), do: [%{row: 0, column: 2}]
